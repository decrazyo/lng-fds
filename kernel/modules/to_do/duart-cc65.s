...
;;;      - uses only channel A
;;;	23.06.2002
;;;	 - some changes basing on cc65 DUART driver (look for !!!, ???)
...
cklast:		lda  #SND_IRQ|REC_IRQ
	+	and  ISR			;??? should it be: "lda ISR; and #SND_IRQ|REC_IRQ"?
		bne  ckloop
...
normal_mode:
		...
		lda #%00100000
		sta CRA				; reset receiver
		lda #%00110000
		sta CRA				; reset transmiter
		lda #%01010000
		sta OPCR			; set indicator
		lda #%10110000
		sta CRA				; transmit X=0
		lda #%10010000
		sta CRA				; receive X=0
		lda #%00000000
		sta ACR				; set default speed 9600
		lda #%10111011
		sta CSRA			; set default speed 9600
		lda #%00010000
		sta CRA				; reset MR pointer
		lda #%01010011	
		sta MR1A			; 8bit no parity+interrupt [bit6] rxready 1=FIFO FULL
		lda #%00000111
		sta MR2A			; 1 stop bit
